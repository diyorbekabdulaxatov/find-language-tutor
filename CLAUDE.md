...
- **i18n (backend).** `internal/i18n` is gettext-shaped: the English literal at the call site *is* the catalog key; `catalog/ru.json` / `uz.json` map it to a translation. The `Locale()` middleware negotiates `Accept-Language` into the gin context (read by `web.WriteError`, which translates at render time) and the request context (`i18n.FromContext`). Messages with arguments must keep the format string as the key: use `web.BadRequestf` / `WriteErrorf`, or a module `ValidationError` (all embed `i18n.Msg`, rendered via `web.BadRequestErr(c, ve)`). Email templates take the recipient's `users.locale`. **Two tests in `internal/i18n` are the guard**: an AST walk that fails when any user-facing literal lacks a ru/uz entry (`I18N_DUMP=1 go test ./internal/i18n -run Cover -v` lists the source set), and a verb-order check so a translation can't scramble `%d`/`%s` arguments. Adding a message = write it in English at the call site, run the test, add the two translations it names.
- **Teacher media.** A profile's photo / intro video are uploads (`POST /v1/uploads`) referenced by `avatar_asset_id` / `intro_video_asset_id`; the service verifies ownership + content type through the `teachers.FileReader` port and writes the public route path (`/v1/teachers/{slug}/media/{avatar|intro-video}`, no auth, approved profiles only, Range-capable) into the matching `*_url` column, so every existing `avatar_url` reader is untouched. The frontend absolutizes `/v1/...` URLs with `mediaUrl()` (`src/lib/media.ts`) and lets `next/image` fetch them (`dangerouslyAllowLocalIP` is on only when the API is on localhost).
...
**Udemy look-alike** (user's call, 2026-09-18 — replaced the earlier green "bold modern marketplace"). Tokens in `src/app/globals.css`: white pages, `--ink` `#1c1d1f` for text and the dark bands (course hero, "My learning" band, player bar, footer), `--primary` `#a435f0` for the one primary action, `--link` `#5624d0` for links/focus, `--border` `#d1d7dc` hairlines instead of shadows, amber `--star` with a brown `--rating` number, **4px radii** (`--radius`; `rounded-xl/2xl` collapse to 8px), one family — **Inter** (`--font-sans`; `font-display` = same face at 700). The old names (`coral`, `mint`, `brand-tint`, `shadow-soft/card/lift`) still exist and map into this palette so nothing breaks. Buttons are 40/48px with bold labels: `default` purple, `outline` white + ink hairline, `secondary` solid ink. Page containers are `max-w-[1340px]`. Udemy-shaped surfaces: header (wordmark, Explore, pill search, outline Log in + ink Sign up), home (banner card over a photo collage, tabbed language row, "become a teacher" split), search results (toolbar + accordion sidebar + rows), course landing (dark hero band, sticky buy card via row-subgrid, "Course content" accordion, "Student feedback"), player (dark bar, right-rail course content), instructor-style teacher profile. Dark theme is kept, Udemy-flavoured. The `frontend-design` skill in `.claude/skills/` is the general design guide; older "warm paper / marigold / Fraunces" notes in `frontend/README.md` are stale.
