...
> = {
  held: { label: "stateHeld", className: "bg-star/20 text-rating dark:text-star" },
  available: { label: "stateAvailable", className: "bg-accent text-accent-foreground" },
...
export function EarningsPanel() {
  ...
  if (!data || data.lessons.length === 0) {
    return (
      <p className="border border-border px-4 py-16 text-center text-sm text-muted-foreground">
        {t("noEarnings")}
      </p>
    );
  }
            ...
            {data.lessons.map((l) => (
              <tr key={l.bookingId ?? l.courseEnrollmentId}>
                ...
                <td className="px-4 py-3 text-right">
                  <span
                    className={cn(
                      "inline-flex rounded-sm px-2 py-0.5 text-xs font-bold",
