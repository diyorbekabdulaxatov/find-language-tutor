...
type Mailer interface {
	...
	// grading).
	// locale is the student's language (users.locale).
	SubmissionGraded(ctx context.Context, locale, to, studentName, resourceTitle string, score, max *int, feedback string) error
}

// noopMailer is the default before SetMailer.
type noopMailer struct{}

func (noopMailer) SubmissionGraded(context.Context, string, string, string, string, *int, *int, string) error {
	return nil
}
